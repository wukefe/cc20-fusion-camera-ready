...
HorseIR-noopt has the worst performance in most cases showing that optimization is
...
\textit{When we are worse.}
In q14/q22, the fused loop sizes are relatively small with complex expressions in
the body.
For example, the complex code is generated for fusing element-wise and
\texttt{like} functions in q14.

%% For example, the complex code is generated for mixing element-wise functions
%% and the string matching function \texttt{like} in q14.

%% \textit{When we are worse.}
%% In q14/q22, the fused loop sizes are relatively small with complex expressions in
%% the body.
%% For example, element-wise functions and special boolean functions (such as
%% \texttt{like}) are fused in q14.
%% The complex code is generated because it has to adopt the parallel and
%% JIT-based code for string matching while fusing element-wise functions.
...
\refFig{fig:opt_number} shows the number of fused statements using
...
However, the increase in compilation time is associated with the exploitation of additional
