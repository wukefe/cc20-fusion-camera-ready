...
of the boolean selection function \texttt{@compress} with the boolean mask \texttt{a}.
The code generator uses this boolean selection shape for further fusion and
...
For vector functions, the return shape can be:
(1) a parameter shape;
...
Statically known vector lengths provide exact shape propagation rules and
can throw errors at compile time. If one or more argument is a dynamically
...
List functions apply a function (\texttt{@f}) on cells individually and merge the results into a new list.
For example, the \texttt{@each} function is shown in \refFig{fig:list_function}.

\begin{figure}[htbp]
\begin{lstlisting}[language=HorseIR, frame=single, basicstyle=\footnotesize, linewidth=.95\columnwidth]
